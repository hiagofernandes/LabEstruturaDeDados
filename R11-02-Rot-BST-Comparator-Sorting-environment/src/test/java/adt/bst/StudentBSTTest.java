package adt.bst;

import static org.junit.Assert.*;

import java.util.Comparator;

import org.junit.Before;
import org.junit.Test;

import adt.bst.BSTImpl;
import adt.bst.extended.SortComparatorBSTImpl;
import adt.bt.BTNode;

public class StudentBSTTest {

	private SortComparatorBSTImpl<Integer> tree;

	private SortComparatorBSTImpl<Integer> tree2;
	private BTNode<Integer> NIL = new BTNode<Integer>();

	private void fillTree() {
		Integer[] array = { 6, 23, -34, 5, 9, 2, 0, 76, 12, 67, 232, -40 };
		for (int i : array) {
			tree.insert(i);
		}
	}

	@Before
	public void setUp() {
		Comparator<Integer> comparator1 = new Comparator<Integer>() {

			@Override
			public int compare(Integer o1, Integer o2) {

				return Integer.compare(o1, o2); // Mudar isso aqui
			}

		};

		Comparator<Integer> comparator2 = new Comparator<Integer>() {

			@Override
			public int compare(Integer o1, Integer o2) {

				return Integer.compare(o2, o1); // Mudar isso aqui
			}

		};

		tree = new SortComparatorBSTImpl<>(comparator1);
		tree2 = new SortComparatorBSTImpl<>(comparator2);
	}

	@Test
	public void testInit() {
		assertTrue(tree.isEmpty());
		assertEquals(0, tree.size());
		assertEquals(-1, tree.height());

		assertEquals(NIL, tree.getRoot());

		assertArrayEquals(new Integer[] {}, tree.order());
		assertArrayEquals(new Integer[] {}, tree.preOrder());
		assertArrayEquals(new Integer[] {}, tree.postOrder());

		assertEquals(NIL, tree.search(12));
		assertEquals(NIL, tree.search(-23));
		assertEquals(NIL, tree.search(0));

		assertEquals(null, tree.minimum());
		assertEquals(null, tree.maximum());

		assertEquals(null, tree.sucessor(12));
		assertEquals(null, tree.sucessor(-23));
		assertEquals(null, tree.sucessor(0));

		assertEquals(null, tree.predecessor(12));
		assertEquals(null, tree.predecessor(-23));
		assertEquals(null, tree.predecessor(0));
	}

	@Test
	public void testMinMax() {
		tree.insert(6);
		assertEquals(new Integer(6), tree.minimum().getData());
		assertEquals(new Integer(6), tree.maximum().getData());

		tree.insert(23);
		assertEquals(new Integer(6), tree.minimum().getData());
		assertEquals(new Integer(23), tree.maximum().getData());

		tree.insert(-34);
		assertEquals(new Integer(-34), tree.minimum().getData());
		assertEquals(new Integer(23), tree.maximum().getData());

		tree.insert(5);
		assertEquals(new Integer(-34), tree.minimum().getData());
		assertEquals(new Integer(23), tree.maximum().getData());

		tree.insert(9);
		assertEquals(new Integer(-34), tree.minimum().getData());
		assertEquals(new Integer(23), tree.maximum().getData());
	}

	@Test
	public void testSucessorPredecessor() {

		fillTree(); // -40 -34 0 2 5 6 9 12 23 67 76 232

		assertEquals(null, tree.predecessor(-40));
		assertEquals(new Integer(-34), tree.sucessor(-40).getData());

		assertEquals(new Integer(-40), tree.predecessor(-34).getData());
		assertEquals(new Integer(0), tree.sucessor(-34).getData());

		assertEquals(new Integer(-34), tree.predecessor(0).getData());
		assertEquals(new Integer(2), tree.sucessor(0).getData());

		assertEquals(new Integer(0), tree.predecessor(2).getData());
		assertEquals(new Integer(5), tree.sucessor(2).getData());
	}

	@Test
	public void testSize() {
		fillTree(); // -40 -34 0 2 5 6 9 12 23 67 76 232

		int size = 12;
		assertEquals(size, tree.size());

		while (!tree.isEmpty()) {
			tree.remove(tree.getRoot().getData());
			assertEquals(--size, tree.size());
		}
	}

	@Test
	public void testHeight() {
		fillTree(); // -40 -34 0 2 5 6 9 12 23 67 76 232

		Integer[] preOrder = new Integer[] { 6, -34, -40, 5, 2, 0, 23, 9, 12, 76, 67, 232 };
		assertArrayEquals(preOrder, tree.preOrder());
		assertEquals(4, tree.height());

		tree.remove(0);
		assertEquals(3, tree.height());

		tree.remove(2);
		assertEquals(3, tree.height());
	}

	@Test
	public void testRemove() {
		fillTree(); // -40 -34 0 2 5 6 9 12 23 67 76 232

		Integer[] order = { -40, -34, 0, 2, 5, 6, 9, 12, 23, 67, 76, 232 };
		assertArrayEquals(order, tree.order());

		tree.remove(6);
		order = new Integer[] { -40, -34, 0, 2, 5, 9, 12, 23, 67, 76, 232 };
		assertArrayEquals(order, tree.order());

		tree.remove(9);
		order = new Integer[] { -40, -34, 0, 2, 5, 12, 23, 67, 76, 232 };
		assertArrayEquals(order, tree.order());

		assertEquals(NIL, tree.search(6));
		assertEquals(NIL, tree.search(9));

	}

	@Test
	public void testSearch() {

		fillTree(); // -40 -34 0 2 5 6 9 12 23 67 76 232

		assertEquals(new Integer(-40), tree.search(-40).getData());
		assertEquals(new Integer(-34), tree.search(-34).getData());
		assertEquals(NIL, tree.search(2534));
	}

	@Test
	public void testSort() {

		fillTree(); // -40 -34 0 2 5 6 9 12 23 67 76 232

		Integer[] reverseOrder = { 232, 76, 67, 23, 12, 9, 6, 5, 2, 0, -34, -40 };
		assertArrayEquals(reverseOrder, tree.reverseOrder());

		Integer[] outOrder = { 5, 3, 8, 4 };
		Integer[] inOrder = { 3, 4, 5, 8 };

		assertArrayEquals(inOrder, tree.sort(outOrder));

		assertEquals(new Integer(5), tree.root.getData());

	}

	@Test
	public void newInsert() {

		tree2.insert(8);
		tree2.insert(10);

		assertEquals(new Integer(10), tree2.getRoot().getLeft().getData());

		tree2.insert(3);

		assertEquals(new Integer(3), tree2.getRoot().getRight().getData());

		tree2.insert(5);

		assertEquals(new Integer(5), tree2.getRoot().getRight().getLeft().getData());

	}

	@Test
	public void testNewSearch() {

		tree2.insert(8);
		tree2.insert(10);

		assertEquals(new Integer(10), tree2.search(10).getData());

		assertEquals(NIL, tree2.search(2534));

		assertEquals(new Integer(8), tree2.search(8).getData());

		tree2.insert(5);

		assertEquals(new Integer(5), tree2.search(5).getData());
	}

	@Test
	public void testSortComparatorInvertido() {

		tree2.insert(1);
		tree2.insert(2);
		tree2.insert(3);

		Integer[] inOrder = { 3, 2, 1 };

		assertArrayEquals(inOrder, tree2.sort(inOrder));
		
		assertArrayEquals(new Integer[]{1, 2, 3}, tree2.reverseOrder()); 

	}

	@Test
	public void testReverseOrder() {

		tree.insert(1);
		tree.insert(2);
		tree.insert(3);

		Integer[] orderMin = { 1, 2, 3 };
		assertArrayEquals(orderMin, tree.order());

		Integer[] reverseOrderMin = { 3, 2, 1 };
		assertArrayEquals(reverseOrderMin, tree.reverseOrder());

		tree.remove(1);
		tree.remove(2);

		Integer[] reverseOrderOnly3 = { 3 };
		assertArrayEquals(reverseOrderOnly3, tree.reverseOrder());
		assertArrayEquals(reverseOrderOnly3, tree.order());

		tree.remove(3);

		// Manipulando mais numeros

		fillTree();

		Integer[] order = { -40, -34, 0, 2, 5, 6, 9, 12, 23, 67, 76, 232 };
		assertArrayEquals(order, tree.order());

		Integer[] reverseOrder = { 232, 76, 67, 23, 12, 9, 6, 5, 2, 0, -34, -40 };
		assertArrayEquals(reverseOrder, tree.reverseOrder());

		tree.remove(12);

		Integer[] reverseOrder1 = { 232, 76, 67, 23, 9, 6, 5, 2, 0, -34, -40 };
		assertArrayEquals(reverseOrder1, tree.reverseOrder());

		tree.insert(300);

		Integer[] reverseOrder2 = { 300, 232, 76, 67, 23, 9, 6, 5, 2, 0, -34, -40 };
		assertArrayEquals(reverseOrder2, tree.reverseOrder());

		tree.insert(78);

		Integer[] reverseOrder3 = { 300, 232, 78, 76, 67, 23, 9, 6, 5, 2, 0, -34, -40 };
		assertArrayEquals(reverseOrder3, tree.reverseOrder());

		Integer[] order1 = { -40, -34, 0, 2, 5, 6, 9, 23, 67, 76, 78, 232, 300 };
		assertArrayEquals(order1, tree.order());

	}
}

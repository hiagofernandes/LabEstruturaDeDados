...
public class RepositorioProdutoNaoPerecivelArray {
	 ...
	 * inicial é -1 para indicar que nenhum produto foi ainda guardado no
	 * array.
	 */
	private int index = -1;
	...
	public void atualizar(ProdutoNaoPerecivel produto) {
		int indication = procurarIndice(produto.getCodigo());
		this.produtos[indication] = produto;
	}
	...
	public void remover(int codigo) {
		int indication = procurarIndice(codigo);
		if (indication != -1)
			this.produtos[indication] = null;
		for (int i = indication; i < produtos.length - 1; i++) {
			produtos[indication] = produtos[indication + 1];
		}
		--index;
	}
